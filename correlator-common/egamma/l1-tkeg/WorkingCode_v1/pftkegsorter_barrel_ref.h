...
const unsigned int NOBJTOSORT = 10;
const unsigned int NOBJSORTED = 16;
...
namespace l1ct {
  ...
  PFTkEGSorterBarrelEmulator(const unsigned int nObjToSort = 10, const unsigned int nObjSorted = 16)
    ...
    void toFirmware_pho(const std::vector<OutputRegion> &outregions,
			EGIsoObj (&photons_in)[2][NOBJTOSORT]) const {
      for (unsigned int ib = 0; ib < 2; ib++) {
        for (unsigned int io = 0; io < nObjToSort_; io++) {
          EGIsoObj pho;
          if (io < outregions[ib].egphoton.size()) {
            pho = outregions[ib].egphoton[io];
	  }
          else
            pho.clear();
    ...
    void toFirmware_ele(const std::vector<OutputRegion> &outregions,
			EGIsoEleObj (&eles_in)[2][NOBJTOSORT]) const {
      for (unsigned int ib = 0; ib < 2; ib++) {
        for (unsigned int io = 0; io < nObjToSort_; io++) {
          EGIsoEleObj ele;
	  if (io < outregions[ib].egelectron.size()) {
	    ele = outregions[ib].egelectron[io];
	  }
          else
            ele.clear();
    ...
    template <typename T>
      ...
      for (unsigned int i : region_index) {
    	...
    	if (debug_) std::cout<<"objs size "<<objs.size()<<"\n";
    	resize_input(objs);
    	objs_in.push_back(objs);
    	if (debug_) std::cout<<"objs (re)size and total objs size "<<objs.size()<<" "<<objs_in.size()<<"\n";
      }
  ...
  private:
    ...
    template <typename T>
      ...
      out = in_region1;
      if (debug_) for (const auto &tmp : out) std::cout<<"out "<<tmp.hwPt<<" "<<tmp.hwEta<<" "<<tmp.hwPhi<<"\n";
	...
	if (in_objs.size() % 2 == 1) merge_regions(to_merge[to_merge.size()-2], to_merge[to_merge.size()-1], out, nObjSorted_);//16, 10, 16 //e.g. is size is 3, we merge 01 and 2, if size is 7, we merge 012345 and 6
