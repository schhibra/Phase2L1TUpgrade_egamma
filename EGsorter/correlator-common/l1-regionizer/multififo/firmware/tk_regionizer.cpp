...
void tk_route_link2fifo_unpacked(const l1ct::TkObj & in, l1ct::TkObj & center, bool & write_center, l1ct::TkObj & after, bool & write_after, l1ct::TkObj & before, bool & write_before) {
    ...
    const auto newEta = in.hwEta + etaShift, vtxEta = in.hwVtxEta() + etaShift; 
    bool valid   = (in.hwPt != 0);
    bool ineta_c = (newEta <= +(PFREGION_ETA_SIZE/2+PFREGION_ETA_BORDER)) && (newEta >= -(PFREGION_ETA_SIZE/2+PFREGION_ETA_BORDER));
    bool ineta_v = (vtxEta <= +(PFREGION_ETA_SIZE/2+PFREGION_ETA_BORDER)) && (vtxEta >= -(PFREGION_ETA_SIZE/2+PFREGION_ETA_BORDER));
    ...
    bool in_cent_v = (in.hwVtxPhi() <= +(PFREGION_PHI_SIZE/2+PFREGION_PHI_BORDER)) && (in.hwVtxPhi() >= -(PFREGION_PHI_SIZE/2+PFREGION_PHI_BORDER));
    bool in_next_v = in.hwVtxPhi() >= +(PFREGION_PHI_SIZE/2-PFREGION_PHI_BORDER);
    bool in_prev_v = in.hwVtxPhi() <= -(PFREGION_PHI_SIZE/2-PFREGION_PHI_BORDER);
    l1ct::TkObj inlocal = in; 
    inlocal.hwEta = newEta;
#ifdef TKROUTER_NOVTX
    write_center = valid && ineta_c && in_cent; center = inlocal; 
    write_after  = valid && ineta_c && in_next;  after = phiShifted(inlocal, -PFREGION_PHI_SIZE); 
    write_before = valid && ineta_c && in_prev; before = phiShifted(inlocal, +PFREGION_PHI_SIZE);
#else
    write_center = valid && (ineta_c && in_cent || ineta_v && in_cent_v); center = inlocal; 
    write_after  = valid && (ineta_c && in_next || ineta_v && in_next_v);  after = phiShifted(inlocal, -PFREGION_PHI_SIZE); 
    write_before = valid && (ineta_c && in_prev || ineta_v && in_prev_v); before = phiShifted(inlocal, +PFREGION_PHI_SIZE);
#endif
}
