...
namespace l1ct {
  ...
  PFTkEGSorterBarrelEmulator(const unsigned int nObjToSort = 10, const unsigned int nObjSorted = 16)
    ...
    void toFirmware_pho(const OutputRegion &outregions,
			EGIsoObj (&photons_in)[NOBJTOSORT]) const {
    ...
    void toFirmware_ele(const OutputRegion &outregions,
			EGIsoEleObj (&eles_in)[NOBJTOSORT]) const {
      for (unsigned int i = 0; i < nObjToSort_; i++) {
	EGIsoEleObj ele;
